...
**Версия:** 1.2.0 · **Клиент:** Кафе Bella Vista
...
- [x] Устаревшие кнопки «Отмена» автоматически убираются при смене шага, `/start` и повторном вводе — в чате не остаётся кнопок с неактуальным действием
...
|------------|----------|:-----------:|:------------:|
| `BOT_TOKEN` | Токен бота от @BotFather | ✅ (без него бот не запустится) | — |
| `ADMIN_CHAT_ID` | Telegram ID администратора для получения заказов | Рекомендуется. Если не задан — уведомления не отправляются, заказы пишутся только в `orders.log` | — |
...
- Добавьте новую категорию — добавьте новый блок по образцу выше. Кнопки категорий и блюд строятся из `MENU` автоматически, править клавиатуры не нужно.
